...
static QUESTION_PATTERNS: LazyLock<Vec<Regex>> = LazyLock::new(|| {
    vec![
        ...
        Regex::new(r"(?i)\b(comment|pourquoi|quand|où|qui|quel|quelle|quels|quelles|combien)\b")
            .expect("Invalid regex: French question words"),
        Regex::new(r"(?i)\b(est-ce que|c'est quoi|qu'est-ce)\b")
            .expect("Invalid regex: French question phrases"),
        Regex::new(r"(?i)\b(peux-tu|pourrais-tu|sais-tu|connais-tu)\b")
            .expect("Invalid regex: French can-you patterns"),
        // English
        Regex::new(r"(?i)^(what|why|how|when|where|who|which|whose)\b")
            .expect("Invalid regex: English question words"),
        Regex::new(r"(?i)\b(can you|could you|do you|would you|is it|are there)\b")
            .expect("Invalid regex: English can-you patterns"),
        Regex::new(r"(?i)\b(what's|what is|how do|how can|how to)\b")
            .expect("Invalid regex: English what-how patterns"),
    ]
});
...
static CODE_PATTERNS: LazyLock<Vec<Regex>> = LazyLock::new(|| {
    vec![
        // Code keywords
        Regex::new(
            r"(?i)\b(code|coder|programmer|script|fonction|function|class|classe|method|méthode)\b",
        )
        .expect("Invalid regex: code keywords"),
        Regex::new(r"(?i)\b(debug|debugger|bug|error|erreur|fix|fixer|corriger)\b")
            .expect("Invalid regex: debug keywords"),
        Regex::new(r"(?i)\b(compile|compiler|build|test|tester|refactor|refactorer)\b")
            .expect("Invalid regex: build keywords"),
        Regex::new(r"(?i)\b(variable|array|tableau|loop|boucle|condition|if|else)\b")
            .expect("Invalid regex: programming constructs"),
        // Programming languages
        Regex::new(r"(?i)\b(rust|python|javascript|typescript|java|c\+\+|go|ruby|php|sql)\b")
            .expect("Invalid regex: programming languages"),
        Regex::new(r"(?i)\b(react|vue|angular|node|express|django|flask|rails)\b")
            .expect("Invalid regex: frameworks"),
        // Code artifacts
        Regex::new(r"```").expect("Invalid regex: code block marker"),
        Regex::new(r"(?i)\b(api|endpoint|route|module|package|crate|library)\b")
...
static CREATIVE_PATTERNS: LazyLock<Vec<Regex>> = LazyLock::new(|| {
    vec![
        // French
        Regex::new(r"(?i)^(écris|écrire|rédige|rédiger|compose|composer)\b")
            .expect("Invalid regex: French creative verbs"),
        Regex::new(r"(?i)\b(imagine|imaginer|invente|inventer|crée une histoire|raconte)\b")
            .expect("Invalid regex: French imagination verbs"),
        Regex::new(r"(?i)\b(poème|poésie|chanson|histoire|conte|récit|roman)\b")
            .expect("Invalid regex: French creative nouns"),
        // English
        Regex::new(r"(?i)^(write|compose|draft|create a story)\b")
            .expect("Invalid regex: English creative verbs"),
        Regex::new(r"(?i)\b(imagine|invent|story|tale|poem|poetry|song|lyrics)\b")
            .expect("Invalid regex: English imagination words"),
        Regex::new(r"(?i)\b(creative|fiction|narrative|essay|article)\b")
            .expect("Invalid regex: English creative nouns"),
    ]
});
...
static GREETING_PATTERNS: LazyLock<Vec<Regex>> = LazyLock::new(|| {
    vec![
        // French
        Regex::new(r"(?i)^(bonjour|bonsoir|salut|coucou|hey|yo)\b")
            .expect("Invalid regex: French greetings"),
        Regex::new(r"(?i)^(bienvenue|rebonjour)\b").expect("Invalid regex: French welcome"),
        // English
        Regex::new(r"(?i)^(hello|hi|hey|greetings|good morning|good afternoon|good evening)\b")
            .expect("Invalid regex: English greetings"),
        Regex::new(r"(?i)^(howdy|what's up|sup)\b")
            .expect("Invalid regex: English informal greetings"),
    ]
});
...
static FAREWELL_PATTERNS: LazyLock<Vec<Regex>> = LazyLock::new(|| {
    vec![
        ...
        Regex::new(r"(?i)\b(au revoir|à bientôt|à plus|bye|ciao|adieu|bonne nuit)\b")
            .expect("Invalid regex: French farewells"),
        // English
        Regex::new(r"(?i)\b(goodbye|bye|farewell|see you|take care|good night)\b")
            .expect("Invalid regex: English farewells"),
    ]
});
...
static ANALYSIS_PATTERNS: LazyLock<Vec<Regex>> = LazyLock::new(|| {
    vec![
        // French
        Regex::new(r"(?i)\b(analyse|analyser|compare|comparer|évalue|évaluer|examine|examiner)\b")
            .expect("Invalid regex: French analysis verbs"),
        Regex::new(r"(?i)\b(résume|résumer|synthèse|synthétise|récapitule)\b")
            .expect("Invalid regex: French summary verbs"),
        Regex::new(r"(?i)\b(différence|avantage|inconvénient|pour et contre)\b")
            .expect("Invalid regex: French comparison nouns"),
        // English
        Regex::new(r"(?i)\b(analyze|analyse|compare|evaluate|examine|assess|review)\b")
            .expect("Invalid regex: English analysis verbs"),
        Regex::new(r"(?i)\b(summarize|summarise|synthesize|recap|overview)\b")
            .expect("Invalid regex: English summary verbs"),
        Regex::new(r"(?i)\b(difference|pros and cons|advantages|disadvantages)\b")
            .expect("Invalid regex: English comparison nouns"),
    ]
});
...
static TRANSLATION_PATTERNS: LazyLock<Vec<Regex>> = LazyLock::new(|| {
    vec![
        // French
        Regex::new(r"(?i)\b(traduis|traduire|traduction|traduisez)\b")
            .expect("Invalid regex: French translation verbs"),
        Regex::new(r"(?i)\b(en français|en anglais|en espagnol|en allemand)\b")
            .expect("Invalid regex: French language targets"),
        // English
        Regex::new(r"(?i)\b(translate|translation)\b")
            .expect("Invalid regex: English translation verbs"),
        Regex::new(r"(?i)\b(to french|to english|to spanish|to german|into)\b")
            .expect("Invalid regex: English language targets"),
    ]
});
...
static EXPLANATION_PATTERNS: LazyLock<Vec<Regex>> = LazyLock::new(|| {
    vec![
        // French
        Regex::new(r"(?i)\b(explique|expliquer|clarifie|clarifier|détaille|détailler)\b")
            .expect("Invalid regex: French explanation verbs"),
        Regex::new(r"(?i)\b(c'est quoi|définition|signifie|veut dire)\b")
            .expect("Invalid regex: French definition phrases"),
        // English
        Regex::new(r"(?i)\b(explain|clarify|elaborate|detail)\b")
            .expect("Invalid regex: English explanation verbs"),
        Regex::new(r"(?i)\b(what does.*mean|definition|meaning of)\b")
            .expect("Invalid regex: English definition phrases"),
    ]
});
...
static HELP_PATTERNS: LazyLock<Vec<Regex>> = LazyLock::new(|| {
    vec![
        ...
        Regex::new(r"(?i)\b(j'ai besoin d'aide|peux-tu m'aider|besoin d'assistance)\b")
            .expect("Invalid regex: French help phrases"),
        // English
        Regex::new(r"(?i)\b(help me|i need help|assist|assistance|support)\b")
            .expect("Invalid regex: English help words"),
        Regex::new(r"(?i)\b(can you help|could you help)\b")
            .expect("Invalid regex: English help phrases"),
    ]
});
