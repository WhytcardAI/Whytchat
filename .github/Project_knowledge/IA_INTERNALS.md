...
*   **Modèle par défaut** : `Qwen 2.5 7B Instruct` (Quantized Q4_K_M).
    *   *Pourquoi ce choix ?* Excellent support du français, très performant pour le code et capable de suivre des instructions complexes avec une empreinte mémoire raisonnable (4.6GB).
...
2.  **Semantic Fallback (Embeddings)** - *~50ms*
...
*   **Embeddings** : `fastembed` (Rust wrapper pour ONNX Runtime).
    *   Modèle : `bge-small-en-v1.5` (Transforme le texte en vecteurs de 384 dimensions).
    *   *Note* : Malgré son nom "en" (English), il performe décemment sur le multilingue pour des tâches simples.
...
2.  Découpage (Chunking) : Par ligne (actuellement, voir *Limitations*).
3.  Vectorisation : `Text -> [0.12, -0.45, ...]`
4.  Indexation : LanceDB écrit les données sur le disque.

### Workflow de Recherche
1.  Question utilisateur -> Vecteur Query.
2.  Recherche ANN (Approximate Nearest Neighbor) dans LanceDB.
3.  Récupération des 3-5 chunks les plus proches (Top-K).
4.  Injection dans le prompt système :
    > "Utilise les informations de contexte suivantes pour répondre : [CONTEXTE]"
...
---
*Basé sur ARCHITECTURE_BRAIN_V2.md - Novembre 2025*