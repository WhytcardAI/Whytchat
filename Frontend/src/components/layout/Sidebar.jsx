        ...
        <div className="space-y-0.5 px-2">
          {items.map((conv) => (
            ...
            >
              ...
              <div
                className={`absolute left-3 z-10 ${selectedConversationIds.includes(conv.id) ? "opacity-100" : "opacity-0 group-hover:opacity-100"} transition-opacity duration-200`}
              ...
              <div
                className={`shrink-0 transition-opacity duration-200 ${selectedConversationIds.includes(conv.id) ? "opacity-0" : "group-hover:opacity-0"}`}
              >
                {conv.type === "meeting" ? (
                  <Users className="w-4 h-4 text-purple-400" />
                ) : (
                  <MessageSquare
                    className={`w-4 h-4 ${currentConversationId === conv.id ? "text-primary" : "text-muted-foreground/70 group-hover:text-foreground"}`}
                  />
                )}
              </div>
              ...
              >
                {editingId === conv.id ? (
                  ...
                  <div className="flex flex-col gap-0.5">
                    <span
                      className={`text-xs truncate transition-colors ${currentConversationId === conv.id ? "font-medium" : "font-normal"}`}
                    >
                      {conv.title}
                    </span>
