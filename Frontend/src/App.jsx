  ...
  return (
    <div className="flex flex-col h-screen w-screen bg-transparent text-foreground overflow-hidden font-sans selection:bg-primary/20 selection:text-primary">
      ...
      <div className="flex flex-1 overflow-hidden relative p-3 gap-3">
        ...
        <div className={`absolute left-3 top-6 z-30 transition-transform duration-500 ${isSidebarOpen ? 'translate-x-[265px]' : 'translate-x-0'}`}>
           <button
        ...
        <div className={`hidden lg:block absolute right-3 top-6 z-30 transition-transform duration-500 ${isContextPanelOpen ? '-translate-x-[305px]' : 'translate-x-0'}`}>
          <button
            onClick={toggleContextPanel}
            className="p-1.5 rounded-full glass-hover text-muted-foreground hover:text-foreground transition-colors"
            title={isContextPanelOpen ? "Collapse context" : "Expand context"}
          >
            {isContextPanelOpen ? <ChevronRight className="w-4 h-4" /> : <ChevronLeft className="w-4 h-4" />}
          </button>
        </div>
      ...
      <Toaster theme={theme === "light" ? "light" : "dark"} position="top-center" toastOptions={{
